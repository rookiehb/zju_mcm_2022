...
def trans_gold (x):
    return x*941.45 + 1125.7

def trans_bit (x):
    return x*62960.25 + 594.08
#Read
filename_B = 'data/BCHAIN-MKPRU.csv'
filename_G = 'data/LBMA-GOLD.csv'
filename_GP = 'data/GoldPrediction.csv'
filename_BP = 'data/BitcoinPrediction.csv'
...
with open(filename_GP) as fgp:
    reader = csv.reader(fgp)
    header_row = next(reader)

    price_gold_predict_1 = []
    price_gold_predict_2 = []
    price_gold_predict_3 = []
    for row in reader:
        price_gold_predict_1.append(trans_gold(float(row[0])))
        price_gold_predict_2.append(trans_gold(float(row[1])))
        price_gold_predict_3.append(trans_gold(float(row[2])))

with open(filename_BP) as fbp:
    reader = csv.reader(fbp)
    header_row = next(reader)
    ...
    price_bit_predict_3 = []
    for row in reader:
        price_bit_predict_1.append(trans_bit(float(row[0])))
        price_bit_predict_2.append(trans_bit(float(row[1])))
        price_bit_predict_3.append(trans_bit(float(row[2])))
...
while(day+Tp < L):
    ...
    maxMG = max(price_gold_predict_1[day],price_gold_predict_2[day], price_gold_predict_3[day])
    #maxMB = predict_B_3d_highst[day+Tp]
    maxMB = max(price_bit_predict_1[day], price_bit_predict_2[day], price_bit_predict_3[day])
    '''
...
fig, ax =plt.subplots()
#plt.plot(date, predict_B_3d_highst[0:L-Tp],linewidth = 0.6,  c='green')
#plt.plot(date, R, linewidth = 0.3, c='blue')
plt.plot(date, props[0:L-Tp], linewidth = 0.4, c='red')
#plt.plot(date, price_bit_predict_1[0:-2], linewidth = 0.3, c='blue')
